  ...
  return (
    <MainColumn width={680}>
      ...
      <section className="mt-10">
        <h2 className="text-[16px] font-medium leading-[20px] text-[var(--color-fg)]">Topics</h2>
      ...
      <section className="mt-12">
        <h2 className="text-[16px] font-medium leading-[20px] text-[var(--color-fg)]">People to follow</h2>
        <ul className="mt-4 divide-y divide-[var(--color-border)]">
          {people.map((u) => (
            <li key={u.id} className="flex items-start gap-4 py-5">
              <Avatar user={u} size={48} />
              <div className="min-w-0 flex-1">
                <Link href={`/@${u.username}`} className="text-[16px] font-medium leading-[20px] text-[var(--color-fg)] hover:underline">{u.name}</Link>
      ...
      <section className="mt-12 pb-16">
        <h2 className="text-[16px] font-medium leading-[20px] text-[var(--color-fg)]">Publications</h2>
        <ul className="mt-4 divide-y divide-[var(--color-border)]">
          {pubs.map((p) => (
            <li key={p.id} className="py-5">
              <Link href={`/${p.slug}`} className="text-[16px] font-medium leading-[20px] text-[var(--color-fg)] hover:underline">{p.name}</Link>
